 ...
 */
export async function loadAssets(): Promise<MockPayload> {
...
/** Dev-only: load assets and store them for dispatchMockMessages(). */
export async function initBrowserMock(): Promise<void> {
  mockPayload = await loadAssets();
}

/** The ordered asset-load messages the renderer expects, built from a payload.
 *  Shared by the dev mock and the SSE transport so the load sequence lives in
 *  one place. */
export function assetLoadMessages(payload: MockPayload): unknown[] {
  return [
