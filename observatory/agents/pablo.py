...
def _fail(draft_id: str, platform: str, error: str) -> PabloResult:
    event_log.append_event(
        "pablo", "pablo.failed",
        draft_id=draft_id, platform=platform or None,
        payload={"error": error},
    )
    return PabloResult(ok=False, error=error)


async def publish_draft(draft_id: str) -> PabloResult:
    draft = drafts_store.get_draft(draft_id)
    if not draft:
        return _fail(draft_id, "", f"draft not found: {draft_id}")
    ...
    try:
        ...
        return _fail(draft_id, platform, f"postiz request failed: {e}")
    except Exception as e:
        logger.error("Pablo: unexpected: %s", e)
        return _fail(draft_id, platform, f"unexpected: {e}")
    ...
    if not postiz_post_id:
        return _fail(draft_id, platform, f"postiz returned no post id: {data}")
